...
for z in include_zs:
    ...
    backend_filename = f'/Users/eb35267/Desktop/code/temp/samples_{run_name}_{z}.h5'
    ...
    ev_fig.savefig(f'evolving_UVLF_{z}.png')

    # Plot walkers & burn-in
    walker_fig = eplots.walkers(my_UVLF, best_fit)
    walker_fig.savefig(f'walkers_{z}.png')

    # Parameter table
    table = eplots.make_table([best_fit], param_labels, ['best fit'], [bounds])
    dfi.export(table, f'table_{z}.png', table_conversion = 'matplotlib', use_mathjax = True, dpi = 200)

    os.replace(backend_filename, f'samples_{z}.h5') # Move the h5 file into current directory once the run is done (not before since I have that directory
